...
namespace SPHFluid
{
    public class SPHParticleSystemSharedMemory : MonoBehaviour
    {
        ...
        void InitializeComputeBuffers()
        {
            ...
            List<ParticleForce> particlesForce = new List<ParticleForce>();
            List<ParticleDensity> particlesDensity = new List<ParticleDensity>();
            ...
            for (float y = MinInitBoundary.y + d; y <= MaxInitBoundary.y - d; y += d)
            {
                for (float x = MinInitBoundary.x + d; x <= MaxInitBoundary.x - d; x += d)
                {
                    ...
                    pf.Acceleration = Vector2.zero;
                    particlesForce.Add(pf);

                    ParticleDensity pd = new ParticleDensity();
                    pd.Density = 0.0f;
                    particlesDensity.Add(pd);
                }
            }
            ...
            SPHParticlesRead.SetData(particles.ToArray());
            SPHParticlesWrite.SetData(particles.ToArray());
            SPHParticlesForce.SetData(particlesForce.ToArray());
            SPHParticlesDensity.SetData(particlesDensity.ToArray());
        }
